...
test('documents a mandatory replacement and rights review before production release', () => {
  assert.match(assetPolicy, /The values in .*business-facts\.json.* are development-demo samples unless their approval source identifies owner confirmation\./);
  assert.match(assetPolicy, /Before production release, replace remaining samples with owner-confirmed values for:/);
